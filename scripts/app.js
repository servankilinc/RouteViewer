...
map.on(L.Draw.Event.CREATED, function (event) {
  ...
  if (geoJson.geometry.type == "Point" && polygons.length > 0) {
    ...
    layer.bindPopup(`Bina Bilgisi: (${_id})`)
    drawnItems.addLayer(layer);
...
$("#btnSavePolyInfos").on("click", function(){
  const poly = polygons.find(p => p.id == $('#modalSetPolyInfo').attr("currentPolyId"));
  poly.name = $('#buildingName').val();
  
  var layer = drawnItems.getLayer($('#modalSetPolyInfo').attr("currentPolyId"));
  layer.setPopupContent(`Bina Bilgisi (${$('#buildingName').val()})`);
...
function sync_path_UI(){
  ...
  paths.map(p => {
    if(p.floor != currentFloor.index){ return; }
...
function sync_polygon_UI(){
  ...
  polygons.map(p => {
    ...
    let btnShow = drawnItems.getLayer(p.id) == null ?
    '<div class="btn btn-primary btn-sm" onclick="ShowLocationOnMap(\'' + p.id + '\')"><i class="fa-solid fa-eye fa-2xs"></i></div>' :
    '<div class="btn btn-secondary btn-sm" onclick="ShowLocationOnMap(\'' + p.id + '\')"><i class="fa-solid fa-eye-slash fa-2xs"></i></div>';
    
    $("#locationList").append(
      '<div class="list-group-item list-group-item-action fw-light d-flex justify-content-between">' +
        '<span>' + p.name + '</span>'+
        '<div class="d-flex gap-1">'+
          '<div class="btn btn-warning btn-sm" onclick="EditInfoLocationOnMap(\'' + p.id + '\')"><i class="fa-solid fa-pen-to-square fa-2xs"></i></div>'+
          btnShow +
          '<div class="btn btn-danger btn-sm" onclick="DeleteLocationOnMap(\'' + p.id + '\')"><i class="fa-solid fa-trash fa-2xs"></i></div>'+
        '</div>'+
      '</div>'
    );
  })
...
window.ShowPathOnMap = function(pID){
  if(drawnItems.getLayer(pID) != null){ // hide then
...
function AddPathToMap(pID){
  ...
  if(selectedPath != -1){
    const layerToInsert = L.geoJSON(selectedPath, {
      onEachFeature: function (feature, layer) {
...
function AddPolygonToMap(pID){
  ...
  if(selectedLocation != -1){
    const layerToInsert = L.geoJSON(selectedLocation, {
      onEachFeature: function (feature, layer) {
...
window.DeletePathOnMap = function(pID){
  ...
  const selectedLayer = drawnItems.getLayer(pID);
  if(drawnItems.getLayer(pID) != null){ // haritadan hide edilmiş zaten
    drawnItems.removeLayer(selectedLayer);
  }
